...
const LOCAL = 'local';
const { tmpDirPath } = makeTmpDir('paraloader-npm-diff');
...
function prepareNPMPackage(revision: string): string {
  if (revision === LOCAL) {
    npm({ cwd: localRepoPath(), quiet: true }).run('build');
    return localRepoPath('dist');
  }
  ...
  npm({ cwd: repoDir, quiet: true }).run('build');
